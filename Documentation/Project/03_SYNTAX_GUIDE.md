...
⚠️ **Важно:** Макрос `logformat` принимает **только 2 аргумента**: формат-строку и значения для форматирования. Для нескольких значений используйте макрос `arg` для объединения их в единый аргумент.
