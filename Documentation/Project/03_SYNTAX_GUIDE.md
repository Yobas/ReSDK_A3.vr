...
Проект использует язык SQF (Scripting Language for Arma 3) с расширениями через систему макросов. В этом руководстве описаны особенности синтаксиса для разных частей проекта.
...
**Важно о макросах OOP:** Имена членов (полей и методов) в макросах OOP должны быть известны на этапе компиляции. Если имя строится динамически во время выполнения, используйте Reflect-версии макросов.
...
// Второй аргумент - переменная с функцией или код
private _myHandler = {
    params ["_data"];
    logformat("Received data: %1", _data);
};
rpcAdd("myAction", _myHandler);
