...
**Важно:** `main` - это защищенная ветка. Прямые коммиты в `main` не допускаются (за исключением обновления файла версии `Src/VERSION`). Все изменения должны проходить через Pull Request'ы.
