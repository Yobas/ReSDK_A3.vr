...
```sqf
// ❌ Неправильно - использует OOP (доступно только на сервере)
// CommonComponents/MyModule.sqf
myModule_process = {
    params ["_obj"];
    callFunc(_obj,doSomething); // OOP доступен только на сервере!
};
...
Порядок загрузки модулей критичен из-за зависимостей между ними. Нарушение порядка может привести к ошибкам "undefined variable" или "Cannot call (nil value)".
...
- Ошибки "Cannot call (nil value)" - модуль пытается вызвать функцию из незагруженного модуля
...
// Зависимости:
// - engine.hpp (базовые макросы)
// - OOP_engine (если используются классы)
// - GameObjects (если работа с игровыми объектами)
...
**В комментариях загрузчика:**

```sqf
// host/init.sqf

// Загрузка OOP Engine (требуется для GameObjects)
loadFile("src\host\OOP_engine\oop_object.sqf");

// Загрузка GameObjects (требуется для NOEngine)
#include "GameObjects\loader.hpp"

// Загрузка NOEngine (зависит от GameObjects)
loadFile("src\host\NOEngine\NOEngineInit.sqf");
```

### Как избежать циклических зависимостей

**Проблема:**
Циклические зависимости возникают, когда модуль A зависит от модуля B, а модуль B зависит от модуля A.

**Решение:**
1. **Вынесите общий функционал в CommonComponents**
   ```sqf
   // Если модули A и B используют общий код, вынесите его в CommonComponents
   // CommonComponents/SharedCode.sqf
   shared_commonFunction = {
       // общий код
   };
   ```

2. **Разделите ответственность модулей**
   - Модуль A отвечает за одну функциональность
   - Модуль B отвечает за другую функциональность
   - Общий функционал в отдельном модуле

3. **Используйте интерфейсы или события**
   - Модули взаимодействуют через события или интерфейсы
...
// ✅ Хорошо - общий код вынесен
// CommonComponents/Shared.sqf - общий код
// ModuleA использует Shared
// ModuleB использует Shared
...
// Зависимости модуля (в порядке загрузки):
// 1. Базовые системы (engine.hpp) - логирование, утилиты
// 2. OOP система (oop.hpp) - если используются классы
// 3. Игровые объекты (GameObjects) - если работа с объектами
// 4. Специализированные модули - если используются

#include "..\engine.hpp"
#include "..\oop.hpp"
#include "..\GameObjects\GameConstants.hpp"

// Убедитесь, что модули загружены в правильном порядке в host/init.sqf
