...
export class QuickJSDebugSession extends SourcemapSession {
	...
	protected async attachRequest(response: DebugProtocol.AttachResponse, args: AttachRequestArguments, request?: DebugProtocol.Request) {
		...
		// Apply defaults for connect mode
		if (!this._commonArgs.mode)
			this._commonArgs.mode = 'connect';
		if (!this._commonArgs.port)
			this._commonArgs.port = 9229;
		if (!this._commonArgs.address)
			this._commonArgs.address = 'localhost';
	...
	protected async launchRequest(response: DebugProtocol.LaunchResponse, args: LaunchRequestArguments) {
		// JSHost patch: launch behaves like attach (no process spawning).
		// The Arma 3 process with js_host plugin is already running.
		this._commonArgs = args;
		this._argsSubject.notify();
	...
	private async sendBreakpointMessage(file: string) {
		...
		// Normalize path: backslashes to forward slashes to match QuickJS filename
